(() => {
  ...
  const fromTable = table => {
    ...
    const I={status:ix("ステータス"),artist:ix("アーティスト"),title:ix("曲名"),fame:ix("知名度"),load:ix("音域・負荷"),identity:ix("自分らしさ"),key:ix("推奨キー"),reason:ix("選曲理由"),test:ix("試唱結果"),retake:ix("再録理由")};
    const v=(r,i)=>i<0?"":(r.c?.[i]?.f ?? r.c?.[i]?.v ?? "");
    return table.rows.filter(r=>String(v(r,I.title)).trim()).map(r=>({
      status:String(v(r,I.status)||"候補").trim(),artist:String(v(r,I.artist)||"").trim(),title:String(v(r,I.title)||"").trim(),
      fame:starCount2(v(r,I.fame)),load:starCount2(v(r,I.load)),identity:starCount2(v(r,I.identity)),
      key:String(v(r,I.key)||"").trim(),reason:String(v(r,I.reason)||"").trim(),test:String(v(r,I.test)||"").trim(),retake:String(v(r,I.retake)||"").trim()
    }));
  };
  ...
  window.syncFromSheet = function(){
    ...
    return new Promise(resolve=>{
      ...
      const done=(ok,err)=>{
        clearTimeout(timer); try{delete window[cb]}catch(e){}; s.remove();
        if(!ok){ console.warn(err); if(typeof setSyncStatus==="function") setSyncStatus("同期失敗","error"); }
        resolve(ok);
      };
      ...
      s.src=url+(url.includes("?")?"&":"?")+"tqx=responseHandler:"+cb+"&_ts="+Date.now();
